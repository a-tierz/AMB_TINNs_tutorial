function [Z, dt] = Animation(ivp,duration,fps,movie,arrow)
%--------------------------------------------------------------------------
...
% Take position and momentum to export state variables to define the
% Lagrangian problem
% Position transformation:
position = [r.*sin(Phi), -r.*cos(Phi)];
% Velocities:
Vx=(Phidot.*r).*cos(Phi)+rdot.*sin(Phi);
Vy=(Phidot.*r).*sin(Phi)-rdot.*cos(Phi);
% State vector 
Z(1,:)=position(:,1);
Z(2,:)=position(:,2);
Z(3,:)=ivp(1).*Vx(:,1);
Z(4,:)=ivp(1).*Vy(:,1);
dt=(duration-0)/(nframes-1);
